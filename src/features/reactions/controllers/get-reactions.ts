...
export class Get {
  public async reactions(req: Request, res: Response): Promise<void> {
    ...
    const reactions: [IReactionDocument[], number] = cacheReactions[0].length ? cacheReactions :
    await reactionService.getPostReactions({postId: new mongoose.Types.ObjectId(postId)}, {createdAt: -1});
    res.status(HTTP_STATUS.OK).json({ message: 'Post reaction',reactions: reactions[0], count: reactions[1] });
  }

  public async singleReactionByUsername(req: Request, res: Response): Promise<void> {
    const { postId, username} = req.params;
    const cacheReactions: [IReactionDocument, number] | [] = await reactionCache.getSingleReactionByUsernameFromCache(postId, username);
    const reactions: [IReactionDocument, number] | []= cacheReactions.length ? cacheReactions : await reactionService.getSinglePostReactionByUsername(postId, username);
